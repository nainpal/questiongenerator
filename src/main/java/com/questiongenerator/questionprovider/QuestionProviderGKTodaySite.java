...
@Component("QuestionProviderGKTodaySite")
public class QuestionProviderGKTodaySite implements QuestionProvider {

    @Override
    public List<Question> scrapeQuestions(Document document, Config.WebSiteConfig webSiteConfig) {

        Stream questionStream = document.getElementsByClass("wp_quiz_question").stream()
                .map(htmlElement -> htmlElement.text()).collect(Collectors.toList()).stream();

        Stream optionsStream = document.getElementsByClass("wp_quiz_question_options").stream()
                .map(htmlElement -> htmlElement.text()).collect(Collectors.toList()).stream();

        Stream answersStream = document.getElementsByClass("ques_answer").stream()
                .map(htmlElement -> htmlElement.text()).collect(Collectors.toList()).stream();

        ArrayList<String> listQuestionOptions = (ArrayList<String>) Streams.
                zip(questionStream, optionsStream, (question, option) -> {
                    return question + "<ChangeTag>" + option;
                })
                .collect(Collectors.toCollection(ArrayList::new));

        ArrayList<String> questionnaire = (ArrayList<String>) Streams.
                zip(listQuestionOptions.stream(), answersStream, (questionOption, answer) -> {
                    return questionOption + "<ChangeTag>" + answer;
                })
                .collect(Collectors.toCollection(ArrayList::new));
        ...
        for (String questionElement : questionnaire) {
            ...
            String ans = answerPart.substring(answerPart.indexOf("Correct Answer:") + 15, answerPart.indexOf(" ["));
            String rightAns = "";
